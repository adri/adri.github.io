...
First I wanted to solve the **"dogpile" effect** which occurs when a cache expires during a traffic spike. Many users refresh the just expired cache value at the same time and increase the load on the database. The more requests hit the database the slower and less responsive 
