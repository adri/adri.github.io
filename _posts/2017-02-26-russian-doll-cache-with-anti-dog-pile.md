...
## Dog Pile Effect
First I wanted to solve the **"dogpile" effect**, also [cache stampede](https://en.wikipedia.org/wiki/Cache_stampede), which occurs when a cache expires during a traffic spike. Many users regenerate the just expired cache value and query the database. The more users hit the database, the slower and less responsive it gets. 
