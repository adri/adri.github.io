...
One way to prevent this is, when only the first user regenerates a cache value while all other users get a stale value until the cache is regenerated by the first user. Serving a stale value is a better tradeoff than a database being overloaded ;-)
