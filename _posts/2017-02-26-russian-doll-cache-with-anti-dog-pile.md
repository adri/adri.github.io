...
First I wanted to solve the **"dogpile" effect**, also [cache stampede](https://en.wikipedia.org/wiki/Cache_stampede) which , then I learned about the **"Russian doll" approach** for caching nested template fragments. 



## Dog Pile Effect
The **"dogpile" effect** occurs when a cache expires during a traffic spike. Many users regenerate the just expired cache value and query the database. The more users hit the database, the slower and less responsive it gets. 
